...
main(void)
{
    ...
    SysCtlPWMClockSet(SYSCTL_PWMDIV_2);  //50MHZ clock for PWM
  ...
  //  PWMOutputState(PWM1_BASE, PWM_OUT_4_BIT, true);// temporary
    ...
    while(1)
    {
        if(drive == 1)
        {
            ...
            {
                PWMOutputState(PWM1_BASE, PWM_OUT_4_BIT, false);    //Shut off PWM
                //need to delay 5 seconds in unlocked state
            }
        }
        /*else
        {   //reset flags
...
void PortEIntHandler(void)
{
    ...
    {
        //Then there was a pin0 interrupt, reduce speed
        back1 = 1;
        back2 = 0;
    }
    else if( (status & GPIO_INT_PIN_1) == GPIO_INT_PIN_1)
    {
        //Then there was a pin1 interrupt, stop motor
        back2 = 1;
        back1 = 0;
    }
